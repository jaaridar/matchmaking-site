...
<<<<<<< HEAD
        // Correct way to sign in as guest and get the user object
        const { user } = await db.auth.signInAsGuest();

        // Use the user object immediately to register player data
        if (user) {
...
=======
        ...
        showStatus('Account created! Setting up your profile...', 'success');
            handleOnAuth(user);
>>>>>>> 1aa21e853b129604ad464e730ea4cc09ecb48790
    } catch (err) {
...
function handleOnAuth(user) {
    ...
    db.subscribeQuery({ players: { $: { where: { id: user.id } } } }, (resp) => {
        ...
        if (players.length > 0) {
            ...
            // Logged in but no player data found yet.
            // This might happen if 'registerPlayerData' hasn't finished or failed.
            // We stay on auth view or show a loading state.
            console.log('[DB] No player profile found for user.');
        }
    });
}
